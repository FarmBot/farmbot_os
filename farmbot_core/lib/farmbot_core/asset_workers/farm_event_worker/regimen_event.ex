defmodule FarmbotCore.FarmEventWorker.RegimenEvent do
  ...
  def handle_info({:ensure_started}, state) do
    if Asset.get_regimen_instance(state.event) do
      ...
      _regimen_instance = Asset.new_regimen_instance!(state.event, %{started_at: state.event.start_time})
  ...
  def handle_info({:ensure_not_started}, state) do
    ...
    if regimen_instance do
      Logger.debug "RegimenInstance shouldn't exist for event: #{inspect(state.event)} Removing."
  ...
  def handle_info({:ensure_unchanged}, state) do
    ...
    if start_times_ok? do
      ...
      Logger.debug "RegimenInstance start time changed for event: #{inspect(state.event)} Recreating."
      Asset.delete_regimen_instance!(regimen_instance)
