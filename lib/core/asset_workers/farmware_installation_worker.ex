defimpl FarmbotCore.AssetWorker, for: FarmbotCore.Asset.FarmwareInstallation do
  ...
  def handle_info(:timeout, %{fwi: %{manifest: nil} = fwi} = state) do
    with {:ok, %{} = manifest} <- get_manifest_json(fwi),
         %{valid?: true} = changeset <-
           FWI.changeset(fwi, %{manifest: manifest}),
      ...
      BotState.report_farmware_installed(
        updated.manifest.package,
        Manifest.view(updated.manifest)
      )
      ...
      error ->
        ...
        error_log(
          fwi,
          "Failed to download Farmware manifest. Trying again in #{timeout}ms #{inspect(error)}"
        )
  ...
  def handle_info(:timeout, %{fwi: %FWI{} = fwi} = state) do
    with {:ok, %{} = i_manifest} <- load_manifest_json(fwi),
         %{valid?: true} = d_changeset <-
           FWI.changeset(fwi, %{manifest: i_manifest}),
         %FWI{} = dirty <- Ecto.Changeset.apply_changes(d_changeset),
         {:ok, n_manifest} <- get_manifest_json(fwi),
         %{valid?: true} = n_changeset <-
           FWI.changeset(fwi, %{manifest: n_manifest}),
      ...
      error ->
        ...
        error_log(
          fwi,
          "failed to check for updates. Trying again in #{timeout}ms #{inspect(error)}"
        )
  ...
  def maybe_update(%{fwi: %FWI{} = installed_fwi} = state, %FWI{} = updated) do
    case Version.compare(
           installed_fwi.manifest.package_version,
           updated.manifest.package_version
         ) do
      ...
      :lt ->
        ...
        with {:ok, zip_binary} <- get_zip(updated),
          ...
          er ->
            ...
            error_log(
              updated,
              "update failed. Trying again in #{timeout}ms #{inspect(er)}"
            )
  ...
  def get_zip("file://" <> path) do
    ...
    with {:ok, files} <- File.ls(path),
         ...
         {:ok, {_path, zip_binary}} <-
           :zip.create(to_charlist(path), file_list, [:memory]) do
      {:ok, zip_binary}
    end
  end
  ...
  def install_farmware_tools(
        %FWI{manifest: %{farmware_tools_version_requirement: _version}} = fwi
      ) do
          ...
          Enum.each(list, fn {filename, data} ->
            out_file =
              Path.join([
                install_dir,
                "farmware_tools",
                Path.basename(to_string(filename))
              ])
