defmodule Farmbot.BotState do
  ...
  def handle_call(:force_state_push, _from, state) do
    new_state =
      update_in(state, [:informational_settings, :cache_bust], fn old ->
        old + 1
      end)
  ...
  def handle_call({:set_user_env, key, val}, _, state) do
    ...
    case Poison.encode(new_user_env) do
      ...
      _ ->
        {:reply, {:error, "user_env must be json encodeable!"}, [], state}
    end
  end
  ...
  defp do_handle(
         data,
         %{informational_settings: %{sync_status: :booting}} = state
       ) do
    do_handle(data, %{
      state
  ...
  defstruct informational_settings: %{
