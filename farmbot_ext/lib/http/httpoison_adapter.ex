defmodule Farmbot.HTTP.HTTPoisonAdapter do
  ...
  defp do_multipart_request({:ok, %Response{status_code: code, body: bin_body}}, http, meta, path)
       when is_2xx(code) do
    with {:ok, body} <- Farmbot.JSON.decode(bin_body),
  ...
  defp finish_upload({:ok, %Response{status_code: code}}, http, atch_url, meta)
       when is_2xx(code) do
    case Farmbot.JSON.encode(%{"attachment_url" => atch_url, "meta" => meta}) do
      {:ok, body} ->
