defmodule Farmbot.Firmware.GCODE.Encoder do
  ...
  def do_encode(:report_position, [x: _] = arg), do: "R15 " <> encode_xyzs(arg)
  def do_encode(:report_position, [y: _] = arg), do: "R16 " <> encode_xyzs(arg)
  def do_encode(:report_position, [z: _] = arg), do: "R16 " <> encode_xyzs(arg)
  ...
  def do_encode(:report_status_value, pv), do: "R33 " <> encode_status_value(pv)
  def do_encode(:report_pin_value, pv), do: "R41 " <> encode_pin_value(pv)
  ...
  def do_encode(:report_position, xyzs), do: "R82 " <> encode_xyzs(xyzs)

  def do_encode(:report_version, [version]), do: "R83 " <> version

  def do_encode(:report_encoders_scaled, xyz), do: "R84 " <> encode_xyzs(xyz)
  def do_encode(:report_encoders_raw, xyz), do: "R85 " <> encode_xyzs(xyz)
  ...
  def do_encode(:movement, xyzs), do: "G00 " <> encode_xyzs(xyzs)
  def do_encode(:movement_single_axis, xyzs), do: "G01 " <> encode_xyzs(xyzs)
  def do_encode(:emergency_unlock, []), do: "F09"
  def do_encode(:write_paramater, pv), do: "F22 " <> encode_pv(pv)
  def do_encode(:read_all_paramaters, []), do: "F20"
  def do_encode(:emergency_lock, []), do: "E"

  defp encode_xyzs(xyzs) do
    Enum.map(xyzs, fn {param, value} ->
  ...
  defp encode_status_value([{p, v}]), do: "P#{p} V#{v}"
  defp encode_pin_value([{p, v}]), do: "P#{p} V#{v}"
end
