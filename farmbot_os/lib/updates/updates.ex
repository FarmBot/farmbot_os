defmodule Farmbot.System.Updates do
  ...
  @update_handler Application.get_env(:farmbot_os, :behaviour)[:update_handler]
  ...
  defmodule Release do
    ...
    def decode_map(%{} = map) do
      ...
      |> Map.update(:assets, [], fn(assets) ->
        Enum.map(assets, fn(asset) ->
          data = Map.take(asset, asset_keys())
          |> Enum.map(fn({key, val}) ->
            {String.to_atom(key), val}
          end)
          struct(Asset, data)
    ...
    def keys do
      [:tag_name, :target_commitish, :name, :draft, :prerelease, :body, :assets] ++
      ["tag_name", "target_commitish", "name", "draft", "prerelease", "body", "assets"]
    end
  ...
  def download_and_apply_update({%Version{} = version, dl_url}) do
    if @update_handler.requires_reboot?() do
      Farmbot.Logger.warn 1, "Can't apply update. An update is already staged. Please reboot and try again."
  ...
  def check_updates(%Release{} = rel, %CurrentStuff{} = current_stuff) do
    ...
    cond do
      ...
      rel.draft ->
        Farmbot.Logger.warn 1, "Not checking draft release."
        nil
      ...
      prerelease and is_beta_release? and beta_opt_in and !commits_equal? ->
        ...
        case Version.compare(current_version, %{release_version | pre: []}) do
          :lt ->
            Farmbot.Logger.debug 3, "Current version (#{current_version}) is less than beta release (#{release_version})"
            try_find_dl_url_in_asset(rel.assets, release_version, current_stuff)
          :eq ->
            if currently_on_beta do
              Farmbot.Logger.debug 3, "Current version (#{current_version}) is equal to beta release (#{release_version})"
              try_find_dl_url_in_asset(rel.assets, release_version, current_stuff)
            else
              Farmbot.Logger.debug 3, "Current version (#{current_version}) is greater than equal to latest beta (#{release_version})"
              nil
            end

          :gt ->
            Farmbot.Logger.debug 3, "Current version (#{current_version}) is greater than latest beta release (#{release_version})"
            nil
        end

      # if the current version is less than the release version.
      !prerelease and version_comp == :lt ->
        Farmbot.Logger.debug 3, "Current version is less than release."
        try_find_dl_url_in_asset(rel.assets, release_version, current_stuff)
      ...
      !prerelease and version_comp == :eq and !commits_equal? ->
        Farmbot.Logger.debug 3, "Current version is equal to release, but commits are not equal."
        try_find_dl_url_in_asset(rel.assets, release_version, current_stuff)
      ...
      true ->
        ...
        Farmbot.Logger.debug 3, "No updates available: \ntarget: #{Farmbot.Project.target()}: \nprerelease: #{prerelease} \n#{comparison_str}"
        nil
    end
  end
  ...
  def try_find_dl_url_in_asset([%Release.Asset{name: name, browser_download_url: bdurl} | rest], %Version{} = release_version_obj, current_stuff) do
    ...
    if match?(^expected_name, name) do
      ...
      Farmbot.Logger.debug 3, "Incorrect asset name for target: #{current_target}: #{name}"
  ...
  def try_find_dl_url_in_asset([], release_version, current_stuff) do
    Farmbot.Logger.warn 2, "No update in assets for #{current_stuff.target()} for #{release_version}"
    nil
  end

  @doc "HTTP request to fetch a Release."
  def get_release_from_url(url) when is_binary(url) do
    Farmbot.Logger.debug 3, "Checking for updates: #{url}"
    case http_adapter().get(url) do
      # This can happen on beta updates, if on an old token
      # and a new beta release was published.
      {:ok, %{status_code: 404}} ->
        Farmbot.Logger.warn 1, "Got a 404 checking for updates: #{url}. Fetching a new token. Try that again"
  ...
  def apply_firmware(is_beta?, file_path, reboot) when is_boolean(is_beta?) do
    Farmbot.Logger.busy 1, "Applying #{@target} OS update (beta=#{is_beta?})"
    before_update()
    case @update_handler.apply_firmware(file_path) do
      :ok ->
        update_config_value(:bool, "settings", "currently_on_beta", is_beta?)
        Farmbot.Logger.success 1, "OS Firmware updated!"
        if reboot do
          Farmbot.Logger.warn 1, "Farmbot going down for OS update."
          Farmbot.System.reboot("OS Firmware Update.")
        end
      {:error, reason} ->
        Farmbot.Logger.error 1, "Failed to apply update: #{inspect reason}"
        {:error, reason}
    end
  end
  ...
  defp maybe_post_update do
    case File.read(update_file()) do
      ...
      {:ok, old_version} ->
        Farmbot.Logger.info 1, "Updating from #{old_version} to #{@current_version}"
        @update_handler.post_update()
      {:error, :enoent} ->
        Farmbot.Logger.info 1, "Updating to #{@current_version}"
      {:error, err} -> raise err
    end
    before_update()
  end
