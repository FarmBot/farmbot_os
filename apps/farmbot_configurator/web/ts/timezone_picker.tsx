...
type TZSelectHandler = (e: string) => void;

interface SorryProps {
    callback: TZSelectHandler;
}

function transferSelectionToUser(callback: TZSelectHandler) {
    return function (e: React.MouseEvent<HTMLSelectElement>) {
        callback(e.currentTarget.value);
    };
}
/** A (quick) Timezone picker until we have time to write a real one with
 * React-Select. Example usage:
 * <Sorry callback={ (tz: string) => { alert(`U picked ${tz}`) }} />
 */
export function Sorry({callback}: SorryProps) {
    return <select id="timezonemenu" onChange={transferSelectionToUser(callback)} >
